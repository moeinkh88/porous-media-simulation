...
grid_size = 28
grid_size2 = 30  # for obstacles (can set both to 28 for perfect match)
...
function place_agents_noobs!(agents, grid, N)
    free = [(i, j) for i in 1:grid_size, j in 1:grid_size]
    shuffle!(rng, free)
    for n in 1:N
        (x, y) = free[n]
        push!(agents, Agent(x, y, false, false, false, false))
        grid[x, y] = true
    end
end
...
function place_agents_obs!(agents, grid, N)
    free = [(i, j) for i in 1:grid_size2, j in 1:grid_size2 if !((i, j) in blocked) && !grid[i, j]]
    shuffle!(rng, free)
    for n in 1:N
        (x, y) = free[n]
...
function simulate_both()
    # Initial state for both
    agents1 = Agent[]
    grid1 = empty_grid(grid_size)
    place_agents_noobs!(agents1, grid1, initial_population)
    agents2 = Agent1[]
    grid2 = empty_grid_obs(grid_size2, blocked)
    place_agents_obs!(agents2, grid2, initial_population)
    # Histories for plotting
    pop_hist1 = Int[]
    pop_hist2 = Int[]
    states1 = Vector{Tuple{Vector{Int},Vector{Int}}}()  # x,y for agents1
    states2 = Vector{Tuple{Vector{Int},Vector{Int}}}()  # x,y for agents2
...
# --------- Run everything ----------
states1, pop_hist1, states2, pop_hist2 = simulate_both()
anim = composite_animation(states1, pop_hist1, states2, pop_hist2, blocked)
gif(anim, "images/comparison_spatial_and_population1.gif", fps=10)
